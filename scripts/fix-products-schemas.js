...
const targetPath = path.resolve(__dirname, '../../system-FE/src/features/products/types/products.schemas.ts');
...
export const productFormSchema = z.object({
  ...
  purchaseCost: z.preprocess(
    ...
    z.number().min(0, 'سعر التكلفة يجب أن يكون 0 أو أكثر')
  ),
  sellingPrice: z.preprocess(
    (val) => (val === '' || val === null || val === undefined || isNaN(Number(val)) ? 0 : Number(val)),
    z.number().min(0.01, 'سعر البيع يجب أن يكون أكبر من 0')
  ),
  minStockLevel: z.preprocess(
    (val) => (val === '' || val === null || val === undefined || isNaN(Number(val)) ? 5 : Number(val)),
    z.number().min(0, 'حد الطلب الأدنى يجب أن يكون 0 أو أكثر')
  ),
  wholesalePrice: z.preprocess(
    (val) => (val === '' || val === null || val === undefined || isNaN(Number(val)) ? null : Number(val)),
    z.number().min(0, 'سعر الجملة لا يمكن أن يكون سالباً').nullable().optional()
  ),
  isWholesaleAvailable: z.boolean().default(false),
  initialStock: z.preprocess(
    (val) => (val === '' || val === null || val === undefined || isNaN(Number(val)) ? 0 : Number(val)),
    z.number().min(0, 'الرصيد الافتتاحي لا يمكن أن يكون سالباً').optional()
  ),
});
...
console.log('products.schemas.ts updated successfully with robust numeric preprocessing.');
