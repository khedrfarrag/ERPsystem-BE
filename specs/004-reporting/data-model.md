# Data Models & Projections: Phase 4 — Reporting & Business Analytics

## Report Query & Filter Parameters

### 1. `ReportDateRangeFilter`
- `From` (`DateTimeOffset?`): Start timestamp (inclusive).
- `To` (`DateTimeOffset?`): End timestamp (inclusive).
- `Format` (`string?`): Output format (`"json"` default, or `"csv"`).

### 2. `SalesFilterParameters`
- `From` (`DateTimeOffset?`)
- `To` (`DateTimeOffset?`)
- `CustomerId` (`Guid?`)
...
### 5. Cash Flow & Register Audit DTOs
```csharp
